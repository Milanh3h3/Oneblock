execute unless entity @s[tag=lotery_answered] run tellraw @s {"text":"You chose 3"}
...
execute unless entity @s[tag=lotery_answered] run execute if score @e[type=armor_stand,tag=oneblock_event_manager,limit = 1] rng matches 1 run execute as @s run function oneblock:events/lottery/bad_reward
execute unless entity @s[tag=lotery_answered] run execute if score @e[type=armor_stand,tag=oneblock_event_manager,limit = 1] rng matches 2 run tellraw @s [{"text":"You got "},{"text":"NOTHING", "bold": true}]
execute unless entity @s[tag=lotery_answered] run execute if score @e[type=armor_stand,tag=oneblock_event_manager,limit = 1] rng matches 3 run execute as @s run function oneblock:events/lottery/good_reward
